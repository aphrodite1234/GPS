...
/**
 * Created by MaiBenBen on 2019/6/1.
 */
public class SaveToExcel {
    static HashMap map = new HashMap(); 	/*	 * 这个更全	 */
    ...
    public static void modifyDirectly2(File inputFile) throws Exception {
        ...
        cell.setCellFormat(cf);        // 直截添加可以覆盖掉
    ...
    public static void modifyDirectly1(File file) {
        try {
            // Excel获得文件
            Workbook wb = Workbook.getWorkbook(file);			// 打开一个文件的副本，并且指定数据写回到原文件
            WritableWorkbook book = Workbook.createWorkbook(file, wb);
            WritableSheet sheet0 = book.getSheet(0);
            sheet0.addCell(new Label(0, 1, "陈小稳"));            // 添加一个工作表
    ...
    public static void buildNewFormTemplete(File inputFile, File outputFile) {
        try {
            // Excel获得文件
            Workbook wb = Workbook.getWorkbook(inputFile);			// 打开一个文件的副本，并且指定数据写回到原文件
    ...
    public static void drawRect(WritableSheet sheet, int x, int y, int width, int height, jxl.format.BorderLineStyle style, jxl.format.Colour BorderColor, Colour bgColor) throws WriteException {
